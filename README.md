...
서버가 직접 서빙하는 운영자 화면. SSH 없이 여기서 다 합니다. 주소 `https://…/bugfix/ui/` (nginx 가 `/bugfix/` → `/api/` 면 자동, 직접이면 `http://서버:8790/api/ui/`), 키는 `ADMIN_KEY`.

| 탭 | 하는 일 |
|---|---|
| 리포트 | 전 프로젝트 건수·상태, **지금 진행 중인 작업의 실시간 로그**, 실패/PR 열림/병합 필터·검색, 행을 누르면 뷰어(질문·수정 요청 가능) |
| 프로젝트 | 프로젝트 **추가/수정/삭제** — 저장소·브랜치·자동 병합·CORS·API 키·설명·규약·모듈(검증 명령)·화면 확인·허용 도구·환경변수. 저장하면 `bugfix-kit.yml` 에 쓰고 재시작 없이 적용(.bak 보관) |
| 키·계정 | **GitHub 토큰·운영자 키·테스트 계정(공용/프로젝트별)·프로젝트 전용 토큰·API 키**. 서버 홈의 `github-token`/`default.env`/`projects/<이름>.env` 에 600 으로 저장. 값은 다시 보이지 않고 "설정됨(…끝 4자)" 만 |
| 서버 설정 | Claude 실행 파일·PATH·JAVA_HOME·모델·턴 수·시간 제한·디렉터리, 환경 정보(디스크 사용량 등). 포트만 재시작 필요 |
| 점검 | 버튼 한 번으로 **GitHub 토큰 유효성·저장소 권한 / Claude 설치·로그인 / docker·브라우저 이미지 / 저장소 접근(ls-remote) / 앱 화면 열기(front-check)** |
| 로그 | 서버 로그 꼬리(journalctl), 5초 자동 갱신 |

처음 세팅 순서: 키·계정에 GitHub 토큰 → 프로젝트 추가 → 점검 탭에서 GitHub·저장소·Claude·docker 검사 → 앱에 SDK 두 줄. Claude 로그인만은 터미널에서 `claude login` 이 필요합니다(OAuth).
