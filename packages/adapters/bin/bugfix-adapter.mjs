...
//   npx bugfix-adapter spring --package com.myapp.debug --out src/main/java/com/myapp/debug [--base /debug] [--resources src/main/resources]
...
if (kind !== 'spring') {
  console.log(`사용법:
  npx bugfix-adapter spring --package <자바 패키지> --out <그 패키지 디렉터리> [--base /debug] [--resources src/main/resources]

  spring: BugfixLogAppender.java + BugfixLogController.java 를 --out 에 만들고,
...
for (const f of ['BugfixLogAppender.java', 'BugfixLogController.java']) {
  const src = fs.readFileSync(path.join(here, '..', 'spring', f), 'utf8').replaceAll('__PACKAGE__', pkg).replaceAll('__BASE__', base);
...
console.log(`\n앱의 신고 설정에: backendLogs: () => fetch('<REST 주소>${base}/recent-logs?level=WARN&limit=200').then(r => r.json())`);
